...
We're working hard to make *litecore* the most powerful JavaScript library for working with tealcoin. Our goal is to have *litecore* be a library that can be used by anyone interested in tealcoin, and to level expertise differences with great design and documentation.
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Bitcore methods. For example, calling `JSON.stringify` on a Bitcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
```sh
git remote add tealcoin-project git@github.com:tealcoin-project/litecore.git
git pull --rebase tealcoin-project master
```
...
```
Finally go to [github.com/tealcoin-project/litecore](https://github.com/tealcoin-project/litecore) in your web browser and issue a new pull request.
...
[github.com/tealcoin-project/litecore/issues](https://github.com/tealcoin-project/litecore/issues).
